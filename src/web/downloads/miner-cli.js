...
const randomx_1 = require("../core/randomx");
if (!worker_threads_1.isMainThread) {
    ...
    const threadId = Number(worker_threads_1.workerData?.threadId) || 0;
    const totalThreads = Number(worker_threads_1.workerData?.totalThreads) || 1;
    function runWorkerBatch() {
        ...
        worker_threads_1.parentPort?.postMessage({ type: 'hashes', count: BATCH });
    ...
    async function setupMiner() {
        ...
        let cliMode = process.env.MINING_MODE || '';
        ...
        if (cliAddress) {
            ...
            if (cliThreads && cliThreads >= 1)
                allocatedThreads = Math.min(cliThreads, totalCpus);
            if (cliMode)
                miningMode = cliMode;
            if (cliNode)
                NODE_URL = sanitizeNodeUrl(cliNode);
    ...
    const activityLog = [];
    const activeWorkers = [];
    async function handleFoundShare(nonce, hash, templateIndex) {
        if (!currentTemplate || currentTemplate.index !== templateIndex)
            return;
        const submitPayload = {
    ...
    async function startLocalMiningEngine() {
        ...
        for (let t = 0; t < allocatedThreads; t++) {
            const w = new worker_threads_1.Worker(__filename, {
        ...
        while (isMiningRunning) {
            try {
                ...
                if (newTemplate && newTemplate.headerPrefix) {
                    ...
                    if (isNew) {
                        const seed = randomx_1.CortexRandomX.getSeedForBlock(currentTemplate.index);
